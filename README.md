...
Open **dashboard.cmd** on Windows, or visit **http://localhost:8766** when the dashboard is running. **http://localhost:8766/morning** is the morning brief: what ran last night, every idea tried with its median gain, the analyst verdict, whether anything is publishable, the full run history, and an *awaiting publication* list built from that history. `scripts/install-morning-task.ps1 -Apply` registers `discovery-loop-morning`, which opens that page in the default browser every day at 07:00.
...
The default `scheduled` policy preserves that assigned arm. For a single requested arm, it tries the requested model, then another configured model in that family, then configured models in the other family. The default configured chain is `opus, astra, sol`: every Claude call runs on Opus 5 at `--effort xhigh`, and Fable stays registered but off the chain because it costs twice as much per token and, on 2026-09-16, 11 of 24 Fable generation calls died on the $2 per-call cap. An arm label names a family; the chain decides which model that family runs on, so the Fable arm executes on Opus without counting as a fallback. The nonadaptive `ordered` policy follows its configured chain exactly for generation, critique, and retro, regardless of the requested arm. A `paired` request creates the two independent Fable/Astra proposal streams; other policies are `auto`, `openai_only`, `anthropic_only`, `astra_only`, and `fable_only`. The schedule and dashboard accept `policy`, `chain`, and `disabled_families`; CLI overrides are `--routing`, `--model-chain`, and `--disable-family`.
...
Every physical attempt is recorded. Authentication or unavailable errors open a family breaker; quota, usage-limit, or model-unavailable errors open a model breaker. Breakers that describe a condition which clears on its own expire on a timer: usage and quota limits after an hour, timeouts and capacity errors after twenty minutes, while authentication and model-unavailable hold until a person intervenes. When every route sits behind an expiring breaker, a generation waits for the first one to reopen instead of ending the slot, and only while the deadline still leaves room. The per-run routing journal survives research, review, retro, and resume. A malformed or invalid candidate is a result for evaluation, not a reason to switch models. A CLI that stops itself at its per-call cap is reported as a budget stop, not a transport failure, and is not retried under the same cap. Started attempts reserve and settle allowance, including failed attempts; a call the provider refuses before it generates anything, and every skipped route, consume no allowance. If no enabled route is available, the stage stops with a routing failure rather than using an API key or an unrecorded fallback.
