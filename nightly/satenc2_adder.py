#!/usr/bin/env python3
"""Bit-blasted SAT encoding with binary adder-tree pseudo-boolean sums.

Replaces the PBEnc.seqcounter sum constraints in satenc2.py with a
ripple-carry binary adder tree. For each tensor equation, sums the
2R literals (p_pos[r], ~p_neg[r]) with a chain of half-adders and
constrains the resulting binary value to equal t+R via unit clauses.

Variable cost per equation: ~2 * sum_{j=1}^{2R} bit_length(j) new vars,
vs O(2R * (t+R)) for seqcounter.
"""
import sys, time
sys.path.insert(0, '/home/hatch/workspace/discovery-loop/nightly')
sys.path.insert(0, '/home/hatch/workspace/discovery-loop/problems/matrix_multiplication')
from verify import check
from pysat.formula import CNF, IDPool
from pysat.card import CardEnc, EncType
from pysat.solvers import Cadical103


def half_adder(cnf, pool, a, b):
    """Return (s, co) var ids with s <-> a XOR b, co <-> a AND b. a, b are literals."""
    s = pool.id()
    co = pool.id()
    # s <-> a XOR b
    cnf.append([-s, -a, -b])
    cnf.append([-s, a, b])
    cnf.append([s, -a, b])
    cnf.append([s, a, -b])
    # co <-> a AND b
    cnf.append([-co, a])
    cnf.append([-co, b])
    cnf.append([co, -a, -b])
    return s, co


def sum_equals_adder(cnf, pool, lits, target, false_var):
    """Constrain sum(lits) == target via ripple-carry adder tree.

    lits: list of literals (positive or negative ints).
    Adds half-adder chain accumulating into a binary counter, then
    fixes the output bits to target with unit clauses.
    """
    acc = []  # LSB-first var ids; absent high bits are implicitly 0
    for idx, lit in enumerate(lits):
        cnt = idx + 1
        w = cnt.bit_length()
        new_acc = []
        # bit 0: acc[0] XOR lit
        a0 = acc[0] if len(acc) > 0 else false_var
        s0, carry = half_adder(cnf, pool, a0, lit)
        new_acc.append(s0)
        # bits 1..w-1: propagate carry
        for i in range(1, w):
            ai = acc[i] if i < len(acc) else false_var
            si, carry = half_adder(cnf, pool, ai, carry)
            new_acc.append(si)
        # carry out of bit w-1 dropped: sum <= cnt < 2**w, so it's implied 0
        acc = new_acc
    # fix bits to target
    for i, bit_var in enumerate(acc):
        if (target >> i) & 1:
            cnf.append([bit_var])
        else:
            cnf.append([-bit_var])
    # any bits of target beyond len(acc) must be 0
    tb = target.bit_length()
    assert tb <= len(acc), f"target {target} needs {tb} bits but acc has {len(acc)}"


def build_cnf_adder(n, R, max_nz):
    cnf = CNF()
    pool = IDPool(start_from=1)

    def V():
        return pool.id()

    false_var = V()
    cnf.append([-false_var])

    # entry booleans
    neg = [[[[V() for _ in range(n)] for _ in range(n)] for _ in range(3)] for _ in range(R)]
    pos = [[[[V() for _ in range(n)] for _ in range(n)] for _ in range(3)] for _ in range(R)]
    for r in range(R):
        for k in range(3):
            for a in range(n):
                for b in range(n):
                    cnf.append([-neg[r][k][a][b], -pos[r][k][a][b]])

    # sparsity per factor matrix
    for r in range(R):
        for k in range(3):
            nz = []
            for a in range(n):
                for b in range(n):
                    v = V()
                    cnf.append([-v, neg[r][k][a][b], pos[r][k][a][b]])
                    cnf.append([-neg[r][k][a][b], v])
                    cnf.append([-pos[r][k][a][b], v])
                    nz.append(v)
            card = CardEnc.atmost(lits=nz, bound=max_nz, top_id=pool.top,
                                  encoding=EncType.seqcounter)
            cnf.extend(card.clauses)
            pool.top = max(pool.top, card.nv)

    # tensor identity: for each (a,b,c,d,e,f), sum_r U*V*W = t
    for a in range(n):
        for b in range(n):
            for c in range(n):
                for d in range(n):
                    for e in range(n):
                        for f in range(n):
                            t = 1 if (a == e and b == c and d == f) else 0
                            lits = []
                            for r in range(R):
                                un, up = neg[r][0][a][b], pos[r][0][a][b]
                                vn, vp = neg[r][1][c][d], pos[r][1][c][d]
                                wn, wp = neg[r][2][e][f], pos[r][2][e][f]
                                unz, vnz, wnz = V(), V(), V()
                                for (zz, nn, pp) in ((unz, un, up), (vnz, vn, vp), (wnz, wn, wp)):
                                    cnf.append([-zz, nn, pp])
                                    cnf.append([-nn, zz])
                                    cnf.append([-pp, zz])
                                a_nz = V()
                                cnf.append([-a_nz, unz]); cnf.append([-a_nz, vnz]); cnf.append([-a_nz, wnz])
                                cnf.append([-unz, -vnz, -wnz, a_nz])
                                x = V()
                                cnf.append([-x, -un, -vn]); cnf.append([-x, un, vn])
                                cnf.append([x, -un, vn]); cnf.append([x, un, -vn])
                                odd = V()
                                cnf.append([-odd, -x, -wn]); cnf.append([-odd, x, wn])
                                cnf.append([odd, -x, wn]); cnf.append([odd, x, -wn])
                                p_neg, p_pos = V(), V()
                                cnf.append([-p_neg, a_nz]); cnf.append([-p_neg, odd])
                                cnf.append([-a_nz, -odd, p_neg])
                                cnf.append([-p_pos, a_nz]); cnf.append([-p_pos, -odd])
                                cnf.append([-a_nz, odd, p_pos])
                                lits.append(p_pos)
                                lits.append(-p_neg)
                            # ADDER TREE replaces PBEnc.seqcounter:
                            # sum(lits) = t + R
                            sum_equals_adder(cnf, pool, lits, t + R, false_var)
    return cnf, pool, neg, pos


def solve_sat_adder(n, R, max_nz, timeout=120):
    t0 = time.time()
    cnf, pool, neg, pos = build_cnf_adder(n, R, max_nz)
    t_build = time.time() - t0
    print(f"  CNF: {len(cnf.clauses)} clauses, {pool.top} vars, build {t_build:.1f}s", flush=True)
    t1 = time.time()
    with Cadical103() as s:
        for cl in cnf.clauses:
            s.add_clause(cl)
        ok = s.solve()
        t_solve = time.time() - t1
        if not ok:
            print(f"  UNSAT ({t_solve:.1f}s)", flush=True)
            return None
        model = set(v for v in s.get_model() if v > 0)
    U = [[[0]*n for _ in range(n)] for _ in range(R)]
    Vv = [[[0]*n for _ in range(n)] for _ in range(R)]
    W = [[[0]*n for _ in range(n)] for _ in range(R)]
    for r in range(R):
        for k, M in enumerate((U, Vv, W)):
            for a in range(n):
                for b in range(n):
                    if neg[r][k][a][b] in model:
                        M[r][a][b] = -1
                    elif pos[r][k][a][b] in model:
                        M[r][a][b] = 1
    print(f"  SAT ({t_solve:.1f}s)", flush=True)
    return [[U[r], Vv[r], W[r]] for r in range(R)]


if __name__ == '__main__':
    t0 = time.time()
    fac = solve_sat_adder(2, 7, 2, timeout=120)
    dt = time.time() - t0
    if fac:
        print("feasible:", check(fac, 2)["feasible"], f"total {dt:.1f}s")
    else:
        print("no solution", f"total {dt:.1f}s")
