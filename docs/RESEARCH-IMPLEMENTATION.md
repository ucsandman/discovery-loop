...
`matrix_multiplication` is such a non-trial research slot in `night.json`: a fixed-provider slot (paired) outside the trial cycle, promoted from the ungoverned `smart_loop.py` path so the frontier problem gets the ledger, routing journal, Docker isolation and dashboard evidence. Its manifest uses `CONFIRMATION_ON_DEVELOPMENT`: the confirmation matrix re-runs the development targets under fresh seeds, is classified `same_target_fresh_seed_replication`, and withholds no targets from prompts. Its opt-in `per_target_pareto` comparison policy preserves the ordinary median policy for every other plugin. Development and confirmation accept only when at least one target's median gain reaches the minimum effect, no matched target/seed pair regresses, no candidate evaluation fails, and the required seed count completes. Evidence retains the overall median and adds per-target gains plus a separate selection gain used to choose among candidates.

The same plugin explicitly opts into `solver_islands_v1`. `island_evolution.py` keeps three deterministic, run-local islands with a capacity of three distinct programs each. The frozen incumbent enters only after its independent development evaluation. Independently verified candidate programs may enter and are ranked by development selection gain, even when the normal incumbent gate rejects them because another target regressed. Mutation selects one verified parent. Odd iterations select two distinct parents for crossover when the current island contains them; deterministic elite sharing makes crossover reachable without waiting for six generations. This is program-level evolution over complete `solver.py` files, separate from the worker processes a generated solver may launch internally.

Every island iteration records its operator, island, parent paths and hashes, then writes the fully rendered prompt and hash before any physical provider call. Paired providers receive the same frozen plan and prompt, and neither provider's same-iteration result can enter the other's context. The crossover prompt builder consumes the canonical sanitized context while `_call_with_budget` remains the only provider path. A pending generated response is reused after restart. A call recorded as started without durable output becomes `lost_response_indeterminate` and is not reissued, preserving at-most-once behavior at the cost of one unusable proposal.

The population snapshot lives under development evidence and references only the run's incumbent snapshot or candidate solver files. Resume restores the original incumbent development rows rather than re-running a stochastic baseline, validates the original evaluation settings, population bounds, run-local path containment, file hashes and frozen prompt hashes, then restores the exact pending plan. Confirmation, release validation, incumbent promotion and their outcomes remain outside the population and prompts. The current n=3 incumbent rank 26 is a feasible research parent but does not match the plugin's reference rank 23, so island occupancy is not evidence of a record-level decomposition.

Confirmation can advance the local incumbent without making it publishable. Matrix multiplication currently evaluates release eligibility across every target, so the proven-optimal n=2 calibration prevents the all-target release gate from passing. Publishing an individual record target remains a separate, explicitly reviewed capability and is outside this implementation.
...
Controls persist in `runs/control.json`. Continue clears a pause request without starting work. Morning review is a persistent human bookmark. Schedule tuning validates the same configuration used by the runner. Approvals bind the exact evidence bytes, solver and solution artifacts; they never send or publish by themselves. Selected evidence renders island occupancy and mutation/crossover ancestry from the recorded development population. Evidence without the policy says islands are disabled; an enabled run without a population says that no verified parents were recorded.
...
- A governed matrix-multiplication probe used two paired iterations and four local stub generations over tagged copies of the trusted seed solver. Its 15 real Docker evaluations passed the exact verifier at ranks 7, 26 and 49; evidence recorded mutation/mutation then crossover/crossover, identical prompt hashes within each provider pair, three bounded parents in every island, and no confirmed improvement. Completed-run resume made zero additional model or solver calls. No live provider was called, and these unchanged ranks are not a record claim.
