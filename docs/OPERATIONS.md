...
| `discovery-loop-morning` | 07:00, open http://localhost:8766/morning in the default browser, starting the dashboard first if it is down (`scripts/install-morning-task.ps1 -Apply`) |
...
The morning brief page (`/morning`, `morning_brief.py`) reads `night.json`, `evidence.json`, `retro.json`, `mission.json` and `best-*/scores.json` only. Its *awaiting publication* list has three sources: publishable evidence with no approval under `runs/research/approvals/`, an approval with no bundle under `releases/<problem>/`, and a stored `best-*/scores.json` result that the plugin's `beats()` says clears a public record with no entry in `submitted.json`. Plugins whose stored record is a local baseline (miplib_heur, pglib_opf) are shown under incumbents and never queued. A restarted `discovery-loop-dashboard` task is required after upgrading the dashboard code.
