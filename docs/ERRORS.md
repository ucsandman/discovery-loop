...
- The global pre-commit hook runs `ruff format` and vulture on every staged `.py`, including the promoted `best-*/solver.py`; formatting it breaks the hash bound in `confirmation.json` and vulture blocks on generated code. Commit a promoted incumbent with `--no-verify` (plus the gate-freeze `# GATE_OK` note) after checking `sha256(solver.py) == confirmation.candidate_hash`; never hand-edit a hash-bound solver.

## 2026-09-17: PGLib seed solver at 2,000+ buses

- The seed solver accepted its own output at `verify.TOL` (1e-6) while the loop evaluates at `RELEASE_FEASIBILITY_TOL` (1e-8). On small cases the Newton polish closed the gap by luck (case118 still sat at 2.3e-8); on case2000_goc the worker's solution had 4,296 balance residuals at 7e-7 and scored -1. The seed now verifies at 1e-8 before saving.
- The single-slack Newton polish moves the accumulated mismatch onto one generator: on case2000_goc it pushed pg[23] to 10.8015 against a 10.8 limit (1.5e-3 pu), far beyond the 2e-6 `shrink()` margin. A tight warm PIPS re-solve (FEASTOL 1e-9) from the same point verifies at 8e-13 without the polish; case118 needs the polish after the re-solve (1.9e-9). Both candidates come from one PIPS run and whichever verifies is kept.
- `isolation.run_solver` raises on timeout before copying the output file back, so the whole run is lost even if the solver saved earlier. The seed solver was starting a new 2–3 minute attempt at `--time - 5` and got killed at 345 s with nothing kept. It now refuses to start an attempt unless 1.25x the longest attempt so far still fits in the budget.
- A manual research launch in a lane worktree ended `provider_unavailable`: Codex (astra, sol) was usage-limited until Sep 19, and the claude route failed `--max-budget-usd 2` in 2.5 s. A headless `claude -p` carries ~35k tokens of harness context (~$0.35 on Opus before any prompt; a 40k-token prompt cost $1.11), so a large generation prompt exceeds $2. Manual large-case runs use `--call-budget 5 --routing anthropic_only --model-chain opus`; the nightly's `per_call_budget_usd` of 2.0 has the same ceiling.
- A lane worktree keeps its gitignored `best-<problem>/solver.py` across `git checkout`, so a failed first launch left the OLD seed as the incumbent and the next run never exercised the seed fix. Delete `best-<problem>/solver.py` in the lane after moving it to a new commit.
- The seed's budget guard at `elapsed + 1.6 * longest < time + 30` alone let a `--time 8` run go 32 s; the nightly validation runs at 60 s and the worker kills at +45. The guard is now also bounded by `elapsed < time - 5`.
